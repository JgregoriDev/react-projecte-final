...
const SignupSchema = yup.object().shape({
  ...
  cantitat: yup
    .number()
    .max(5000, "El valor màxim del camp cantitat  es 300")
    .required()
    .positive("El camp cantitat ha de ser un valor positiu")
    .integer(),
  preu: yup
    .number()
    .max(5000, "El valor màxim del camp preu  es 5000")
    .required()
...
const JocForm = ({ props }) => {
  ...
  const conseguirPlataformes = async () => {
    ...
    let response = await fetch("http://127.0.0.9/api/v1/plataformes", {
      method: "GET",
      headers: headersList
    });
  ...
  const conseguirGeneres = async () => {
    ...
    let response = await fetch("http://127.0.0.9/api/v1/generes", {
      method: "GET",
      headers: headersList
    });
  ...
  const insertarJoc = async (token, valor) => {
    ...
    // bodyContent.append("portada",
    //   "/home/lpc/Downloads/Screenshot 2022-10-22 at 20-53-51 the rock dwayne johnson at DuckDuckGo.png");
    //  bodyContent.append("generes", valor.generes);
    //  bodyContent.append("videojoc_plataforma", valor.videojoc_plataforma);
    let response = await fetch("http://127.0.0.9/api/v1/videojoc/nou", {
      method: "POST",
      body: bodyContent,
      headers: headersList
    });
